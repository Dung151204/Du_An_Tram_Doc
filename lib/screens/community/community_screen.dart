...
class _CommunityScreenState extends State<CommunityScreen> {
          ...
          IconButton(
            ...
            onPressed: () {
              ...
              Navigator.push(
                  context,
                  MaterialPageRoute(
                      builder: (_) => const SearchUserScreen())).then(
                  (_) => _loadFollowingIds()); // Load lại danh sách khi quay về
            },
          )
        ],
      ),
                      ...
                      return Center(
                        child: Column(
                          ...
                          children: [
                            ...
                            const Text(
                                "Bạn bè của bạn đang 'im hơi lặng tiếng'...",
                                style: TextStyle(color: Colors.grey)),
                            const SizedBox(height: 8),
                            const Text("Chưa có hoạt động mới nào.",
                                style: TextStyle(color: Colors.grey)),
                          ],
                        ),
                      );
  ...
  Widget _buildActivityCard(BookModel book) {
    ...
    if (isReview && isNote)
      actionText = "vừa đánh giá & ghi chú";
    else if (isReview)
      actionText = "đã đánh giá sách";
                ...
                children: [
                  ...
                  if (book.keyTakeaways.length > 2)
                    Padding(
                      ...
                      child: Text(
                          "+ còn ${book.keyTakeaways.length - 2} ý khác...",
                          style: const TextStyle(
                              color: Colors.blue,
                              fontSize: 12,
    ...
    return FutureBuilder<DocumentSnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        if (snapshot.hasData && snapshot.data!.exists) {
          name = (snapshot.data!.data() as Map<String, dynamic>)['fullName'] ??
              "Người dùng";
        }
